...
export async function request<T>(method: string, url: string, body?: unknown, contentType = 'application/json'): Promise<T> {
  ...
  const response = await fetch(url, {method, headers, cache:'no-store', signal: AbortSignal.timeout(12000), body: body === undefined ? undefined : contentType === 'application/json' ? JSON.stringify(body) : body as string});
